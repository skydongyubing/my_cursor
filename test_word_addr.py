#!/usr/bin/env python3
"""测试字地址格式"""

import serial
import time

PORT = "COM6"
BAUDRATE = 115200
ACK = 0x79

def main():
    print("打开串口...")
    ser = serial.Serial(
        port=PORT,
        baudrate=BAUDRATE,
        bytesize=serial.EIGHTBITS,
        parity=serial.PARITY_EVEN,
        stopbits=serial.STOPBITS_ONE,
        timeout=5.0,
        write_timeout=5.0,
    )
    
    ser.rts = False
    ser.dtr = False
    ser.reset_input_buffer()
    
    print("等待 4 秒...")
    time.sleep(4.0)
    ser.reset_input_buffer()
    
    # Sync + Get
    print("\n1. Sync + Get...")
    ser.write(b"\x00\xFF")
    time.sleep(0.5)
    response = ser.read(20)
    print(f"   收到: {response.hex()}")
    
    # Extended Erase
    print("\n2. Extended Erase...")
    ser.write(b"\x44\xBB")
    time.sleep(0.3)
    response = ser.read(1)
    if response and response[0] == ACK:
        ser.write(b"\xFF\xFF\x00")
        print("   等待擦除完成...")
        time.sleep(30)
        response = ser.read(1)
        print(f"   擦除响应: {response.hex() if response else '超时'}")
    
    time.sleep(2)
    
    # Re-sync
    print("\n3. Re-sync...")
    ser.reset_input_buffer()
    ser.write(b"\x00\xFF")
    time.sleep(0.5)
    response = ser.read(20)
    print(f"   收到: {response.hex()}")
    
    # Read 命令
    print("\n4. Read 命令 0x11...")
    ser.write(b"\x11\xEE")
    time.sleep(0.5)
    response = ser.read(1)
    print(f"   响应: {response.hex() if response else '超时'}")
    
    if response and response[0] == ACK:
        # 测试字地址格式
        # Flash 0x08000000 -> 字地址 0x08000000 / 4 = 0x02000000
        # 或者可能是 0x00000000 (相对地址)
        test_addrs = [
            (0x08000000, "Byte address 0x08000000"),
            (0x02000000, "Word address 0x02000000 (0x08000000/4)"),
            (0x00000000, "Zero address"),
            (0x00010000, "Offset 0x10000 (word)"),
        ]
        
        for addr, label in test_addrs:
            print(f"\n5. 测试 {label}:")
            addr_bytes = bytes([(addr >> 0) & 0xFF, (addr >> 8) & 0xFF, 
                                (addr >> 16) & 0xFF, (addr >> 24) & 0xFF])
            crc = addr_bytes[0] ^ addr_bytes[1] ^ addr_bytes[2] ^ addr_bytes[3]
            addr_cmd = addr_bytes + bytes([crc])
            print(f"   发送: {addr_cmd.hex()}")
            
            ser.reset_input_buffer()
            ser.write(addr_cmd)
            time.sleep(0.5)
            response = ser.read(1)
            print(f"   响应: {response.hex() if response else '超时'}")
            
            if response and response[0] == ACK:
                print(f"   成功! 读取数据...")
                ser.write(bytes([0, 0xFF]))  # 1字节
                time.sleep(0.5)
                response = ser.read(1)
                if response and response[0] == ACK:
                    data = ser.read(1)
                    print(f"   数据: {data.hex()}")
                    break
    
    ser.close()
    print("\n完成!")

if __name__ == "__main__":
    main()
